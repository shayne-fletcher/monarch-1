    ...
    pub(crate) fn footer_text<'a>(job: &Option<Self>, labels: &'a Labels) -> &'a str {
        match job {
            ...
            Some(ActiveJob::PySpy {
                ...
                // PY-8 / recovery: Failed must keep advertising the
                // `P` retry affordance, otherwise a transient
                // provision failure forces the operator to rerun
                // py-spy first just to get the hint back.
                ProvisionState::Failed { .. } => labels.footer_pyspy_can_provision_help_text,
                ProvisionState::Idle => labels.footer_pyspy_help_text,
            },
