...
/// implementing `RemoteActor` and `Binds<A>`, with serializable
/// params:
///
/// ```rust,ignore
...
///
/// Bounds explained:
/// - `RemoteActor`: marks the type as eligible for typed remote
...
/// `RemoteActor` is a marker trait for types that can appear as
...
/// `RemoteActor` so that you can hand out restricted or stable APIs
...
/// `RemoteActor` but is never a `RemotableActor`.
