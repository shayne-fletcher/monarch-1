...
impl<M: ProcManager> Host<M> {
    ...
    pub async fn new_with_default(
        ...
        let service_proc_id = ProcId::Direct(frontend_addr.clone(), SERVICE_PROC_NAME.to_string());
        let service_proc = Proc::new(service_proc_id.clone(), router.boxed());

        let local_proc_id = ProcId::Direct(frontend_addr.clone(), LOCAL_PROC_NAME.to_string());
