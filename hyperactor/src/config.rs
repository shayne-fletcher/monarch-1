...
//!
//! This module provides a centralized, type-safe way to manage
//! configuration settings using the `attrs` system. The system
//! supports multiple sources (YAML files, environment variables,
//! runtime-provided values, and temporary test overrides) with
//! serde-based serialization.
//!
//! Configuration is resolved via a **layered model**: `TestOverride →
//! Runtime → Env → File → Default`.
//!
//! - Reads (`global::get`, `global::get_cloned`) resolve in that
//!   order, with Defaults last.
//! - `global::attrs()` returns a **complete snapshot** of the
//!   effective configuration at call time: it materializes defaults for
//!   keys not set in any layer, and omits meta-only keys (like
//!   `CONFIG_ENV_VAR`) unless explicitly set.
//! - When a hyperactor parent process spawns a bootstrap child to
//!   host procs, it can capture its effective configuration via
//!   `global::attrs()` and pass that snapshot to the child. The child
//!   installs this snapshot as its `Runtime` layer, ensuring that the
//!   parent's configuration values "win" inside the child process.
//!
//! This design provides flexibility (easy overrides in tests, runtime
//! updates, YAML-based configuration) while ensuring type safety and
...
mod tests {
    ...
    fn test_layer_precedence_env_over_file_and_replacement() {
        ...
        global::set(global::Source::File, file);

        // Env sets a different value.
        let mut env = Attrs::new();
        env[CODEC_MAX_FRAME_LENGTH] = 2222;
        global::set(global::Source::Env, env);

        // Env should win over File.
        assert_eq!(global::get(CODEC_MAX_FRAME_LENGTH), 2222);

        // Replace Env layer with a new value.
        let mut env2 = Attrs::new();
        env2[CODEC_MAX_FRAME_LENGTH] = 3333;
        global::set(global::Source::Env, env2);

        assert_eq!(global::get(CODEC_MAX_FRAME_LENGTH), 3333);
    }
    ...
    fn test_runtime_overrides_and_clear_restores_lower_layers() {
        ...
        global::set(global::Source::Runtime, rt);
        ...
        global::clear(global::Source::Runtime);
    ...
    fn test_parent_child_snapshot_as_runtime_layer() {
        ...
        global::set(global::Source::Env, parent_env);

        let parent_snap = global::attrs();

        // "Child" process: start clean, install parent snapshot as
        // Runtime.
        global::reset_to_defaults();
        global::set(global::Source::Runtime, parent_snap);

        // Child should observe parent's effective value (as highest
        // stable layer).
        assert_eq!(global::get(MESSAGE_ACK_EVERY_N_MESSAGES), 12345);
    }
    ...
    fn test_reset_to_defaults_clears_all_layers() {
        ...
        // Sanity: highest wins.
        assert_eq!(global::get(SPLIT_MAX_BUFFER_SIZE), 9);

        // Reset clears all explicit layers; defaults apply.
        global::reset_to_defaults();
        assert_eq!(global::get(SPLIT_MAX_BUFFER_SIZE), 5); // default
    }
    ...
    fn test_get_cloned_resolution_matches_get() {
        ...
        assert!(!global::get(CHANNEL_MULTIPART));
        let v = global::get_cloned(CHANNEL_MULTIPART);
        assert!(!v);
    }
