...
//!   or a subscriber `Some(MeshFailure)` (error or clean
//!   `Stopped`/`NotExist`/`Timeout`) carrying a known `reporting_controller`, and
...
//!   `Subscribe`, then casts readiness. The controller replays a latched terminal
//!   on `Subscribe` and broadcasts a later one, so no terminal state races
//!   registration.
...
//! controller's subscriber stream, which also carries clean terminal states and
//! `None` heartbeats). As the controller's parent it additionally receives the
//! controller's own terminal supervision events. Together these three inputs
...
impl Handler<EnsureRdmaManager> for RdmaManagerOwnerActor {
    async fn handle(
        ...
        // `Subscribe` replays any latched terminal and later ones are broadcast
        // to the now-inserted port, so no terminal state races registration.
...
// `Handler<Option<MeshFailure>>` additionally carries *clean* terminal states
// (`Stopped`/`NotExist`/`Timeout`) and `None` heartbeats (RMO-14). Both attribute
// by the typed `reporting_controller` and fail only the source pending entry
// (RMO-7); an error that arrives on both is absorbed by the terminal-absorbing
// transition (RMO-2). Neither returns `Err`, so a report cannot terminate the
// owner (RMO-4).
#[async_trait]
