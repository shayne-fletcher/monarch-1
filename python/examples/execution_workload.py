...
from inside the handler body -- after the framework's
``_execution_started`` has recorded the invocation and before the handler
blocks -- so the signal means "mesh-admin now reports this invocation",
not merely "user code started". The signal travels over a monarch
``Port`` rather than handler stdout: a spawned proc's stdout reaches the
client on a multi-second aggregation window, too coarse for a handshake.
The main task receives on the port and echoes ``EXEC_ENTERED <id>`` to
its own unbuffered stdout.
"""
...
class BusyActor(Actor):
    ...
    async def hold(self, id: str, entered: "Port[str]") -> None:
        ...
        # already run _execution_started, so the registry reflects this
