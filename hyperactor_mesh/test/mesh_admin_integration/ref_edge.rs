...
//!
//! See MIT-25, MIT-26, MIT-27, MIT-28, MIT-29, MIT-30, MIT-31 in
//! `main` module doc.
...
pub(crate) async fn check(s: &DiningScenario) {
    ...
    // --- MIT-28: double-encoded ref ---
    // Axum's path extraction percent-decodes once, and the handler
    // decodes once more, so a double-encoded ref resolves correctly.
    // Assert non-failure rather than failure — this is correct product
    // behavior.
    let single_encoded = urlencoding::encode(&s.worker);
    let double_encoded = urlencoding::encode(&single_encoded);
    let node: NodePayload = s
        .fixture
        .get_json(&format!("/v1/{double_encoded}"))
        .await
        .unwrap();
    assert_eq!(node.identity, s.worker, "MIT-28");
