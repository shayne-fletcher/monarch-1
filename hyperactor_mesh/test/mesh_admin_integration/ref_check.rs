...
pub(crate) async fn check(s: &DiningScenario) {
    ...
    // MIT-24: discover known actors from proc children by name.
    check_known_actors(&s.fixture, &service, "service").await;
    check_known_actors(&s.fixture, &worker, "worker").await;
}

async fn check_known_actors(fixture: &WorkloadFixture, proc_node: &NodePayload, label: &str) {
    let mut found_host_agent = false;
    let mut found_proc_agent = false;
    for actor_ref in &proc_node.children {
        ...
        let base = actor_base_name(actor_ref);
        if base == "host_agent" {
            found_host_agent = true;
        }
        if base == "proc_agent" {
            found_proc_agent = true;
        }
    }
    assert!(
        found_host_agent || found_proc_agent,
        "MIT-24: {label} proc has neither host_agent nor proc_agent",
    );
}
