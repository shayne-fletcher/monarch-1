...
pub(crate) fn render_diagnostics_pane(frame: &mut ratatui::Frame<'_>, area: Rect, app: &App) {
    ...
    let status_line = if app.diag_running {
        Line::from(vec![
            ...
            Span::styled(format!("{}{}", sep, labels.diag_live_updates), scheme.detail_label),
        ])
    ...
    let title_line = if app.diag_running {
        Line::from(vec![
            Span::styled(labels.pane_diagnostics, Style::default().add_modifier(Modifier::BOLD)),
            Span::styled(format!("{}{}", sep, labels.diag_running), scheme.info),
        ])
    } else if let Some(t) = &app.diag_completed_at {
        Line::from(vec![
            Span::styled(labels.pane_diagnostics, Style::default().add_modifier(Modifier::BOLD)),
            Span::styled(format!("{}{}", sep, labels.diag_completed_at), scheme.detail_label),
            Span::raw(" "),
            Span::styled(t.clone(), scheme.stat_timing),
            Span::styled(format!("{}{}", sep, labels.diag_static_snapshot), scheme.detail_stopped),
        ])
    } else {
