...
//! - **TF-EXTRACT-5 (plain-name-contained):** [`install_plain`] rejects
//!   tool names that contain path separators or `..`, so a malicious
//!   spec cannot redirect a plain artifact outside `bin/`.
...
///
/// Enforces TF-CACHE-2, TF-EXTRACT-4, and TF-EXTRACT-5 for non-archive
/// artifacts.
...
/// Reject tool names that would escape the `bin/` directory.
///
/// TF-EXTRACT-5: a [`crate::ToolSpec`] originates from a caller and may
/// be untrusted. The plain install path joins `bin/` with the spec name
/// directly, so anything other than a single normal path component is
/// refused before any filesystem write occurs.
fn validate_plain_name(name: &str) -> Result<(), ProvisionError> {
    let mut components = Path::new(name).components();
    match (components.next(), components.next()) {
        (Some(Component::Normal(_)), None) => Ok(()),
        _ => Err(ProvisionError::UnsafeArchiveEntry {
            path: name.to_string(),
            reason: "tool name must be a single path component".to_string(),
        }),
    }
}
